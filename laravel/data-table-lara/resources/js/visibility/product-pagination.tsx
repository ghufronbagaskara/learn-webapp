    ...
    <div className="flex flex-col items-center justify-between gap-3 border-t border-white/10 px-5 py-4 sm:flex-row">
      ...
      <p className="text-xs text-white/30">
        {filteredRecords === 0 ? (
          ...
          <>
            Menampilkan {startEntry}–{endEntry} dari {filteredRecords.toLocaleString('id-ID')} entri
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-1">
          <PageButton onClick={() => onPageChange(0)} disabled={pageIndex === 0} label="First">
          ...
          {pageNumbers.map((p, i) =>
            p === '...' ? (
              <span key={`ellipsis-${p}-${i}`} className="px-1 text-xs text-white/20">
                …
              </span>
            ) : (
              <button
