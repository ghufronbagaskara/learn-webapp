                        ...
                        {visibleCols.map((col) => {
                            ...
                            return (
                                <th
                                    ...
                                    onClick={() => col.sortable && onSort(columns.findIndex((c) => c.key === col.key))}
                                >
...
function renderCell(
    ...
    switch (col.key) {
        ...
        case 'sku':
            return <code className="rounded bg-white/5 px-1.5 py-0.5 text-xs text-indigo-300/80">{product.sku}</code>;
        case 'category':
            return <span className="text-slate-400">{product.category}</span>;
        case 'formatted_price':
            return <span className="font-mono font-medium text-emerald-400">{product.formatted_price}</span>;
        case 'stock':
